...
export const [WINDOW_WIDTH, WINDOW_HEIGHT] = width < height ? [width, height] : [height, width];
...
export const scaleFontByHeight = (size: number) => Math.round(size * curDeviceHeightRatio);
export const scaleSizeBorder = (size: number, minVal?: number) => Math.max(scaleSize(size), minVal ? minVal : 1);
export const scaleHeightBorder = (size: number, minVal?: number) => Math.max(scaleHeight(size), minVal ? minVal : 1);
export const scaleWidthBorder = (size: number, minVal?: number) => Math.max(scaleWidth(size), minVal ? minVal : 1);

// sets a minimum value
export const scaleUxSize = (size: number) => Math.max(scaleSize(size), minimumClickableSize);
export const scaleUxHeight = (size: number) => Math.max(scaleHeight(size), minimumClickableSize);
export const scaleUxWidth = (size: number) => Math.max(scaleWidth(size), minimumClickableSize);
...
function dimensions(top: number, right: number = top, bottom: number = top, left: number = right, property) {
	let styles = {};
...
export function boxShadow(color: string, offset: Offset = {height: 2, width: 2}, radius: number = 8, opacity: number = 0.2) {
	return {
		shadowColor: color,
		shadowOffset: offset,
